...
bd.lm = lm(weight ~ ., data = bd)  # fit model with all the predictors
plot(bd.lm)
...
step_forward = stepAIC(bd.lm, direction = "forward")    # best model
step_forward$anova
summary(step_forward)   # details of the best model

# part (b): backward selection
bd.back = ols_step_backward_aic(bd.lm)                # best model
plot(bd.back)
...
plot(High_F ~ Latitude, data=city_2)
